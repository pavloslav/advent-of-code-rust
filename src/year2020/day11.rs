...
mod ferry {
    ...
    impl Model {
            ...
            SHIFTS
                ...
                .map(|(shift_row, shift_col)| {
                    (1..)
                        .map(|distance| {
                            (
                                row as i32 + distance * shift_row,
                                col as i32 + distance * shift_col,
                            )
                        })
                        .take_while(|&(row, col)| {
                            0 <= row
        ...
        fn count(&self, kind: char) -> usize {
            self.seats
                .iter()
                .map(|row| row.iter().filter(|&&cell| cell == kind).count())
                .sum()
        }
