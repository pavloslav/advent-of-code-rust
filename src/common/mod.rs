...
pub fn get_input(url:&str, session:&str) -> std::result::Result<String, Error> {
    log!("Trying url '{}'", url);
...
pub fn get_input_from_ini_with_mod(mod_year: &str, mod_day: &str) -> Result<String> {
...
pub fn get_input_from_ini_with_year(year:&str, day:&str) -> Result<String> {
    ...
    std::fs::read_to_string(&filename).or_else(|file_error| {
        ...
        let section = ini.section(Some(AOC_INI_SECTION)).ok_or(Error::WrongIniFormat)?;
        let url     = format_url(section.get("link_year").ok_or(Error::WrongIniFormat)?, year, &number);
