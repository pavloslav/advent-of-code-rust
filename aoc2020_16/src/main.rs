...
impl Field
{
    fn accepts(&self, value:u64) -> bool {
        (self.from1<=value&&value<=self.to1) || (self.from2<=value&&value<= self.to2)
    }
        ...
        match state {
            ScanType::Class => {
                if !line.is_empty() {
                    ...
                    fields.push(Field {name:parts.next().unwrap().to_owned(),
...
fn process_tickets(s:&str) -> u64 
{
        ...
        match state {
            ...
            ScanType::YourTicket => {
                if line.is_empty() {
                    ...
                    your_ticket = line.split(',').map(|x|x.parse().unwrap()).collect();
                }
            },
            ScanType::NearbyTickets => {
                if line!="nearby tickets:" 
                {
                    let ticket:Vec<_> = line.split(',').map(|x|x.parse().unwrap()).collect();
                    if ticket.iter().all(|&value|fields.iter().any(|field|field.accepts(value))) {
    ...
    let mut possibles:Vec<Vec<bool>> = (0..fields.len()).map(|_|((0..your_ticket.len()).map(|_|true).collect())).collect();

    for field_idx in 0..possibles.len() {
        for place_idx in 0..possibles[field_idx].len() {
            for ticket in nearby_tickets.iter() {
                if possibles[field_idx][place_idx] && !fields[field_idx].accepts(ticket[place_idx]) {
    ...
    loop {
        ...
        for field_idx in 0..possibles.len() {
            if possibles[field_idx].iter().filter(|&&x|x).count() == 1 {
                let true_idx = possibles[field_idx].iter().position(|&x|x).unwrap();
        ...
        for place_idx in 0..possibles[0].len() {
            if possibles.iter().filter(|&x|x[place_idx]).count() == 1 {
                let true_idx = possibles.iter().position(|x|x[place_idx]).unwrap();
    ...
    for (i, f) in fields.iter().enumerate() {
        if f.name.starts_with("departure") {
            ...
            let idx = possibles[i].iter().position(|&x|x==true).unwrap();
...
fn main() {
    let input = aoc::get_input_from_ini_with_year("16","2020").unwrap();
