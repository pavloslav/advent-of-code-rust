...
fn parse_tree(
    input: &mut impl std::iter::Iterator<Item = Result<i32>>,
) -> Result<Tree> {
    let mut tree = Tree::new();
    let child_count = input.next().ok_or(aoc_error!("Empty node!"))??;
    let metadata_size =
        input.next().ok_or(aoc_error!("No metadata size!"))??;
...
pub fn parse_input(input: &str) -> Result<Tree> {
    let mut iter = input
        .split_whitespace()
...
pub fn task1(input: &Tree) -> Result<i32> {
    Ok(input.metadata_sum())
}

pub fn task2(input: &Tree) -> Result<i32> {
    Ok(input.value())
}
