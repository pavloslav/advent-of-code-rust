...
fn str_to_plant(input: &str) -> Result<Vec<bool>> {
    input
        .bytes()
        .map(|c| match c {
            PLANT => Ok(true),
...
pub fn parse_input(input: &str) -> Result<(Vec<bool>, HashSet<Vec<bool>>)> {
    let mut input = input.lines();
    let initial = input.next().ok_or(aoc_error!("No initial state!"))?;
    let initial = str_to_plant(&scan_fmt::scan_fmt!(
        initial,
        "initial state: {}",
        String
    )?)?;
    ...
    let rules: HashSet<Vec<bool>> = input
        ...
        .filter_map(|line| {
            let (from, to) =
                match scan_fmt::scan_fmt!(line, "{} => {}", String, char) {
                    Ok(v) => v,
...
pub fn task1(
    (initial, rules): &(Vec<bool>, HashSet<Vec<bool>>),
) -> Result<isize> {
    let loops = 20;
    ...
    for _ in 0..20 {
        ...
        new_state.extend(
            (0..state.len() - 5).map(|i| rules.contains(&state[i..i + 5])),
        );
...
pub fn task2(
    (_initial, _rules): &(Vec<bool>, HashSet<Vec<bool>>),
) -> Result<usize> {
    Err(aoc_error!("Todo"))
}
